	...
	if !instance.GetStatus().UseImmutableImage {
		reserved = append(reserved,
			...
			reservedEnvVar{
				...
				Default: func(ev *corev1.EnvVar) {
					ev.Value = fmt.Sprintf("%s/v1/deployment/installer/agent/unix/default/latest?Api-Token=$(ONEAGENT_INSTALLER_TOKEN)&arch=ppcle&flavor=default", instance.GetOneAgentSpec().APIURL)
				},
			},
			reservedEnvVar{
